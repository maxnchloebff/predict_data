...
class predict_module():
    def __init__(self,sensordata, sampleRate=0.5):
        self.sensor_data = sensordata
        # 读取图像，并变成numpy类型的 array
        self.im = np.array(Image.open('lena256.bmp'))  # 图片大小256*256

        # 生成高斯随机测量矩阵
        self.sampleRate = sampleRate  # 采样率
        self.Phi = np.random.randn(int(256 * self.sampleRate), 256)
        # 生成稀疏基DCT矩阵
        self.mat_dct_1d = np.zeros((256, 256))
        v = range(256)
        ...
        # 随机测量
        self.img_cs_1d = np.dot(self.Phi, self.im)
        self.reconstuct()
    ...
    def reconstuct(self):
        ...
        Theta_1d = np.dot(self.Phi, self.mat_dct_1d)  # 测量矩阵乘上基矩阵
        for i in range(256):
            print("reconstruct the "+str(i)+"th column")
            column_rec = self.cs_CoSaMP(self.img_cs_1d[:, i], Theta_1d)  # 利用CoSaMP算法计算稀疏系数
            sparse_rec_1d[:, i] = column_rec
        img_rec = np.dot(self.mat_dct_1d, sparse_rec_1d)  # 稀疏系数乘上基矩阵
        # 显示重建后的图片
